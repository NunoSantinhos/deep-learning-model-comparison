...
train_classes = torch.tensor(data['train_classes']).long()

test_img = torch.tensor(data['test_X']).float().permute(0, 3, 1, 2)
test_classes = torch.tensor(data['test_classes']).long()
...
train_dataset = TensorDataset(train_img, train_classes)
train_loader = DataLoader(train_dataset, batch_size=BATCH_SIZE, shuffle=True)
test_dataset = TensorDataset(test_img, test_classes)
...
model_original = MultiClassCNN(input_shape=(64, 64, 3), num_classes=10)
...
with torch.no_grad():
    for inputs, targets in test_loader:
        ...
        correct_predictions_original += (predicted == targets).sum().item()
        total_predictions_original += targets.size(0)

test_accuracy_original = correct_predictions_original / total_predictions_original * 100

torch.save(model_original.state_dict(), 'modelo_multiclass.pth')
print("Modelo completo salvo como 'modelo_multiclass.pth'")

# Avaliação no conjunto de teste do MobileNetV2 pré-treinado
mobilenet = mobilenet_v2(pretrained=True)
mobilenet.classifier[1] = nn.Linear(mobilenet.last_channel, train_classes.max().item() + 1)
...
with torch.no_grad():
    for inputs, targets in test_loader:
        ...
        _, predicted = torch.max(outputs, 1)
        correct_predictions_mobilenet += (predicted == targets).sum().item()
        total_predictions_mobilenet += targets.size(0)

test_accuracy_mobilenet = correct_predictions_mobilenet / total_predictions_mobilenet * 100

# Gráfico comparativo
models = ['Multiclass', 'MobileNetV2']
...
# Salvar o gráfico
plt.savefig('multiclass_vs_mobilenetv2.png')
